...
# block1 = char_block(embedded, [64, 128, 192], filter_length=[3, 3, 3], subsample=[1, 1, 1], pool_length=2)
# block2 = char_block(embedded, [96, 160, 256], filter_length=[5, 5, 3], subsample=[2, 2, 1])
block3 = char_block(embedded, [192, 256, 256, 256],
                    filter_length=[5, 5, 3, 3], subsample=[1, 1, 1, 1], pool_length=[2, 2, 2, 2])
# sent_encode = merge([block2, block3], mode='concat', concat_axis=-1)
# sent_encode = Dropout(drop_p)(sent_encode)
# fc
sent_encode = Dense(512, activation='relu', init='glorot_normal')(block3)
sent_encode = Dropout(drop_p)(sent_encode)
sent_encode = Dense(256, activation='relu', init='glorot_normal')(sent_encode)
sent_encode = Dropout(drop_p)(sent_encode)
encoder = Model(input=in_sentence, output=sent_encode)

# sequence = Masking(mask_value=-1.0)(sequence)
encoded = TimeDistributed(encoder)(sequence)

lstm_h = 100
forwards = LSTM(lstm_h, return_sequences=False, dropout_W=0.1, dropout_U=0.1,
                consume_less='gpu')(encoded)
backwards = LSTM(lstm_h, return_sequences=False, dropout_W=0.1, dropout_U=0.1,
                 consume_less='gpu', go_backwards=True)(encoded)

merged = merge([forwards, backwards], mode='concat', concat_axis=-1)
# output = Dropout(drop_p)(merged)
# output = Dense(128, activation='relu', init='he_normal')(merged)
